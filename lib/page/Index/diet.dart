...
class Diet extends StatefulWidget {
  const Diet(this.employeeProvideContract);
...
//  初始化方法
  ...
  Future<void> _getFoodOrders() async {
                  ...
                  for (var datafood in repfoodOrder.result) {
                    ...
                    setState(() {
                      _listFoodDailyOrder.add(FoodDailyOrder(
                          ...
                          lunch: _list[0] == null || _list[0] == ''
                              ? "無"
                              : _list[0],
                          afTea: _list[1] == null || _list[1] == ''
                              ? "無"
                              : _list[1],
                          meal: _list[2] == null || _list[2] == ''
                              ? "無"
                              : _list[2],
                          date: datafood['date'],
  ...
  Future<void> _getFoodOrder() async {
    final ApiResponse response = await widget.employeeProvideContract.getByUser(); //查询员工身份信息
    if (response.success && isValidList(response.results)) {
      ...
      if (repStuList.success && isValidList(repStuList.results)) {
        for (var datastu in repStuList.result) {
          print(datastu['objectId'].toString()+"-----");
          print("循环--------${a++}");
          stuObject.set('objectId', datastu['objectId']);
          QueryBuilder queryfood = QueryBuilder(ParseObject("FoodDailyOrder"))
            ..whereEqualTo('student', stuObject)
            ..includeObject(['student', 'items']);
          var repfoodOrder = await queryfood.query();
          if (repfoodOrder.statusCode == 200) {
            if (repfoodOrder.result != null) {
              List<String> _list = [];
              num i=0;
              for (var food in repfoodOrder.results.first['items']) {
                print("循环=======${b++}");
                print(food['items']['objectId']);
              ...
              setState(() {
                _listFoodDailyOrder.add(FoodDailyOrder(
                    objectId: repfoodOrder.results.first['objectId'],
                    items: repfoodOrder.results.first['items'],
                    lunch: _list[0] == null || _list[0] == '' ? "無" : _list[0],
                    afTea: _list[1] == null || _list[1] == '' ? "無" : _list[1],
                    meal: _list[2] == null || _list[2] == '' ? "無" : _list[2],
                    date: repfoodOrder.results.first['date'],
                    stuName: datastu['member']['displayName'],
                    packaged: repfoodOrder.results.first['packaged']));
              });
            }
          }
        }
